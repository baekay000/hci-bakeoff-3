...
export default function Proto1() {
    const sketch2 = (p5: p5) => {
        ...
        p5.setup = () => {
            p5.createCanvas(800, 800); //Sets the size of the app. You should modify this to your device's native size. Many phones today are 1080 wide by 1920 tall.
        ...
        p5.draw = () => {
            ...
            if (finishTime!=0)
            {
                ...
                p5.text("Trials complete!",400,200); //output
                p5.text("Total time taken: " + (finishTime - startTime),400,220); //output
                p5.text("Total letters entered: " + lettersEnteredTotal,400,240); //output
                p5.text("Total letters expected: " + lettersExpectedTotal,400,260); //output
                p5.text("Total errors entered: " + errorsTotal,400,280); //output
                let wpm = (lettersEnteredTotal/5.0) / ((finishTime - startTime)/60000.); //FYI - 60K is number of milliseconds in minute
                p5.text("Raw WPM: " + wpm,400,300); //output
                let freebieErrors = lettersExpectedTotal*.05; //no penalty if errors are under 5% of chars
                p5.text("Freebie errors: " + p5.nf(freebieErrors,1,3),400,320); //output
                let penalty = p5.max(errorsTotal-freebieErrors, 0) * .5;
                p5.text("Penalty: " + penalty,400,340);
                p5.text("WPM w/ penalty: " + (wpm-penalty),400,360); //yes, minus, because higher WPM is better
            ...
            if (startTime!=0)
            {
                ...
                p5.fill(255, 0, 0);
                p5.rect(600, 600, 200, 200); //draw next button
                p5.fill(255);
                p5.text("NEXT > ", 650, 650); //draw next label
        ...
        function computeLevenshteinDistance(phrase1: string, phrase2: string) //this computers error between two strings
        {
            // let distance = new int[phrase1.length + 1][phrase2.length + 1];
            let distance: number[][] = [];
            let tempArr: number[] = [];
            for (let j = 0; j < phrase1.length + 1; ++j) {
                tempArr.push(0);
            }
            ...
            for (let i = 1; i <= phrase1.length; i++)
                for (let j = 1; j <= phrase2.length; j++)
                distance[i][j] = p5.min(p5.min(distance[i - 1][j] + 1, distance[i][j - 1] + 1), distance[i - 1][j - 1] + ((phrase1.charAt(i - 1) == phrase2.charAt(j - 1)) ? 0 : 1));
